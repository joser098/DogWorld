...
const getAllOrByName = async (name) => {
    ...
    if (!name) {
        ...
        dogsFound?.map(dog => { // Crear el formato correcto para el cliente
            const dogToAdd = {
                id: dog.id,
                image: {
                    url: dog.image
                ...
                temperament: `${dog.Temperaments.map(temperament => ` ${temperament.name}`)}` //Obtener temperamentos de la tabla intermedia
            }
            dogsFoundInDB.push(dogToAdd);
        });

        const { data } = await axios(`https://api.thedogapi.com/v1/breeds`); //obtenemos todos los dogs de la Api
...
///////// SI SE OBTIENE NOMBRE /////////
    ...
    const { data } = await axios(`https://api.thedogapi.com/v1/breeds/search?q=${nameCapitalized}`); //Obtenemos todos los dogs con el nombre

    const dogsFoundinApi = []

    for (const dog of data) { // Iteramos los dogs para buscar su imagen con Id de referencia
        if(dog.hasOwnProperty('reference_image_id')){ // Comprobar que los perritos tengas id de referencia
