...
export const validations = (dogToCreate) => {
    ...
    const checkName = (name) => {
        ...
        if(!/^[A-Za-z\s]+$/.test(name)){
            errors = "Name is obligatory and should be letters and spaces"
        };
        if(name.length > 15){
    ...
    const checkHeight = (height_min, height_max) => {
        if(!height_max || !/^[0-9]+$/.test(height_max)){
            errors = 'Maximun height should not be empty and should be numbers' 
        };
        if(height_max < 15 || height_max > 110){
            errors = 'Maximun height should be between 15 and 110' 
        };
        if(!height_min || !/^[0-9]+$/.test(height_min)){
            errors = 'Minimun height should not be empty and should be numbers' 
        }
    ...
    const checkWeight = (weight_min, weight_max) => {
        if(!weight_max || !/^[0-9]+$/.test(weight_max)){
            errors = 'Maximun weight should not be empty and should be numbers' 
        };
        if(weight_max < 3 || weight_max > 90){
            errors = 'Maximun weight should be between 3 and 90' 
        };
        if(!weight_min || !/^[0-9]+$/.test(weight_min)){
            errors = 'Minimun weight should not be empty and should be numbers' 
        }
    ...
    // Temperaments
    const checkTemperaments = (temperaments) => {
        if(temperaments.length < 1){
            errors = 'Should add at least one temperament'
        };
    }
    ...
    const checkLifeSpan = (life_span_min, life_span_max) => {
        if(!life_span_max || !/^[0-9]+$/.test(life_span_max)){
            errors = 'Maximun life_span should not be empty and should be numbers' 
        };
        ...
        if(!life_span_min || !/^[0-9]+$/.test(life_span_min)){
            errors = 'Minimun life_span should not be empty and should be numbers' 
        }
    ...
    checkTemperaments(dogToCreate.temperaments)
